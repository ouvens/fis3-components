 /*
 * 1. remove touch event handle (mobile event)
 * 2. remove nav handler
 */
(function(root, factory) {
 ...
 /*               $htmlBody.css({
        ...
        */
        ...
        var firstSpecialActionExcute = {};
        firstSpecialActionExcute.down = firstSpecialActionExcute.up = false;


        // ie9 will be executed，event listen bug
        // main event listen handler
            ...
            if(options.autoScrolling && !controlPressed){
                ...
                if(canScroll){
                    ...
                    if(isAccelerating){
                        ...
                        // ie8-9 not trigger onFirstScroll
                        // firsr page has special action, hack it.
                        // console.log(timeDiff, direction, firstSpecialActionExcute, activeSection.hasClass(options.firstSpecialClass))
                        // animation remains 1s
                        if(!lte9 && !firstSpecialActionExcute[direction] && options.firstSpecialClass && activeSection.hasClass(options.firstSpecialClass)){
                            if(timeDiff > 1000) {
                                prevTime = curTime;
                                // current direction
                                firstSpecialActionExcute[direction] = true;
                                // reverse direction
                                firstSpecialActionExcute[direction == 'down' ? 'up' : 'down'] = false;
                                $.isFunction(options.onFirstScroll) && options.onFirstScroll(direction)
                            }                           

                        }else{
                            if(timeDiff > 1000) {
                                prevTime = curTime;
                                scrolling(direction, scrollable);
                            }
                        }
                    }
                }
        ...
        */
        $document.on('click', SECTION_NAV_SEL + ' a', function(e){
            e.preventDefault();
        ...
        */
        $document.on('click', SLIDES_NAV_LINK_SEL, function(e){
            e.preventDefault();
        ...
        */
         ...
         */
        $(SECTION_SEL).on('click', SLIDES_ARROW_SEL, function() {
        ...
        */
        function removeMouseWheelHandler(){
            if (document.addEventListener) {               
                ...
                document.removeEventListener('wheel', MouseWheelHandler, false); //Firefox, IE9          
