...
(function (root, factory) {
    ...
    function Validator() {
        var validator = this.validator = {
            ...
            parentSelector: '.mod-form__line',
            /** 验证提示选择器, 用于将验证的提示或结果展示, 没有时会自动添加 */
            resultSelector: '.field-help',
            ...
            limit: function (str, $item) {
                str = str.toString();
                if ($item.data('trim')) str = $.trim(str);
                var length = 0;
                ...
                if (min && length < notAsciiWidth * min) {
                    validator.msg = (max ? ['请填写', min, '-', max, '个字'] : ['请至少填写', min, '个字']).join('');
                    return false;
                }
            ...
            getMsgHtml: function (msg) {
                return '<i></i><p>{{msg}}</p>'.replace(/{{msg}}/g, msg || '');
            },
            ...
            if_not_null_then: function (str, $item) {
            ...
            bind: function ($item) {
                if ($item.closest(validator.parentSelector).find(validator.resultSelector).length === 0) {
                    ...
                    var $msg = '<div class="' + validator.resultSelector.replace(/^\./, '') + '"></div>';
